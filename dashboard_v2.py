...
4 tabs:
  ...
  Tab 4 — Simulate Call    (pick a scenario → generate + analyze live)
...
def _run_pipeline_on_file(audio_path: str, agent_id: str, model_size: str):
    """Run the pipeline on a single file and return the result dict."""
    try:
        import yaml
        ...
        # Load config and override model size
        cfg = load_config("config.yaml")
        cfg["whisper"]["model"]          = model_size
        cfg["diarization"]["enabled"]    = False   # skip for speed
        ...
        return {"record": record, "elapsed": elapsed, "success": True}
...
def _display_call_result(record, elapsed: float):
    """Display a CallRecord in a structured way."""
    st.success(f"Analysis complete in {elapsed:.1f}s")
    st.markdown("---")

    # KPIs
    c1,c2,c3,c4 = st.columns(4)
    with c1: st.metric("Duration",    f"{record.duration_sec:.1f}s")
    ...
    st.text_area("Full transcript", value=record.full_transcript,
    ...
    if record.all_entities:
        ...
        ent_df = pd.DataFrame(record.all_entities)
